...
/** Executes an animation action.
  ...
  *         \li \b false if an error happend
  *****************************************************************************/
bool CalMixer::executeAction(int id, float delayIn, float delayOut, float weightTarget, bool autoLock, bool extract_root_motion )
{
  // get the core animation
  ...
  pAnimationAction->execute(delayIn, delayOut, weightTarget, autoLock, extract_root_motion);
...
bool CalMixer::removeAction(int id, float delayOut)
{
  ...
  while (iteratorAnimationAction != m_listAnimationAction.end())
  {
    ...
    {
      if (delayOut > 0) {
        (*iteratorAnimationAction)->remove(delayOut);
      }
      else {
        // found, so remove
