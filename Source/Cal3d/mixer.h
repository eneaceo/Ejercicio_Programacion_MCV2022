...
public:
  ...
  bool executeAction(int id, float delayIn, float delayOut, float weightTarget = 1.0f, bool autoLock = false, bool extract_root_motion = false);
  bool removeAction(int id, float delayOut);
  ...
  CalVector getAndClearDeltaLocalRootMotion() {
    CalVector new_delta = root_motion_delta;
