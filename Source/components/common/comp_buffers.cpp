    ...
    if (cte) {
      ...
      if (jval.is_object()) {
        if (jval.count("create_texture")) {
          ...
          bool is_ok = texture->create(xres, yres, fmt, CTexture::CREATE_WITH_COMPUTE_SUPPORT);
          assert(is_ok);
      ...
      // check if it's a static texture resource if ...
      // outTexture : "data/texture/nice.dds"
