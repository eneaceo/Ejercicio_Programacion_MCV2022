...
namespace particles
{
    ...
    TParticle CEmitter::generate(MAT44 ownerTransform) const
    {
        ...
        p.lifetime = 0.f;
        p.maxLifetime = lifetime + randomFloat(-lifetimeVariation, lifetimeVariation);
        p.size = size.get(0.f);
    ...
    VEC3 CEmitter::generatePosition() const
    {
        switch (type)
        {
            case TEmitterType::Box:
            {
                ...
                return VEC3(randomFloat(-size.x, size.x), randomFloat(-size.y, size.y), randomFloat(-size.z, size.z));
            }
            case TEmitterType::Circle:
            {
                const float radius = std::get<float>(typeSize);
                return yawToVector(randomFloat(-M_PI, M_PI)) * randomFloat(0.f, radius);
            }
            case TEmitterType::CirclePerimeter:
            {
                const float radius = std::get<float>(typeSize);
                return yawToVector(randomFloat(-M_PI, M_PI)) * radius;
            }
            case TEmitterType::Sphere:
            {
                const float radius = std::get<float>(typeSize);
                return yawPitchToVector(randomFloat(-M_PI, M_PI), randomFloat(-M_PI, M_PI)) * radius;
            }
            default:;
        }
