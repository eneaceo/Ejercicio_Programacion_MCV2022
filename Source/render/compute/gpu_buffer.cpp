...
bool CGPUBuffer::create(uint32_t new_bytes_per_elem, uint32_t new_num_elems, bool is_indirect) {
  ...
  HRESULT hr = Render.device->CreateBuffer(&bd, nullptr, &buffer);
  if (FAILED(hr))
    return false;
...
bool CGPUBuffer::create(const json& j) {
  ...
  bool is_ok = create(bytes_per_elem, num_elems, is_indirect);
  ...
  if (data_type_name == "MAT44") {
    ...
    if (ImGui::SmallButton("Randomize...")) {
      ...
      for (uint32_t i = 0; i < num_elems; ++i)
        transforms.push_back(MAT44::CreateTranslation(randomFloat(-rad, rad), 0.0f, randomFloat(-rad, rad)));
