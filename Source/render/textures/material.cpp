...
bool CMaterial::createFromJson(const json& j) {
  ...
  std::string roughness_name = j.value("roughness", "data/textures/white.dds");
  roughness = Resources.get(roughness_name)->as<CTexture>();

  std::string metallic_name = j.value("metallic", "data/textures/black.dds");
  metallic = Resources.get(metallic_name)->as<CTexture>();
  ...
  if (casts_shadows) {
    std::string shadows_mat_name = j.value("shadows_mat", "data/materials/shadow_caster.mat");
...
void CMaterial::cacheSRVS()
{
  ...
  if(roughness)
    shader_resource_views[TS_ROUGHNESS] = roughness->getShaderResourceView();
  if( metallic)
    shader_resource_views[TS_METALLIC] = metallic->getShaderResourceView();
  if(emissive)
...
void CMaterial::activate() const {
  ...
  //// This can be collapsed in a single call to DX, so we activate 5 textures 
  //// with consecutive id's at once
  //if (albedo)
  //  albedo->activate(TS_ALBEDO);
  //if (normal)
  //  normal->activate(TS_NORMAL);
  //if (roughness)
  //  roughness->activate(TS_ROUGHNESS);
  //if (metallic)
  //  metallic->activate(TS_METALLIC);
  //if (emissive)
  // emissive->activate(TS_EMISSIVE);
  assert(TS_ALBEDO == 0);
...
bool CMaterial::renderInMenu() const {
  ...
  if (ImGui::TreeNode("Textures..."))
  {
    ...
    if (normal)
      normal->renderInMenu();
    if (roughness)
      roughness->renderInMenu();
    if (metallic)
      metallic->renderInMenu();
    if (emissive)
