...
// ----------------------------------
// Fake forward references needed by some MF headers
typedef struct {} *LPRGNDATA;
...
public:
  ...
  void update( float elapsed ) {
    ...
    if (pVideoSample) {
      ...
      if (!target_texture) {
        target_texture = new CTexture();
        int texture_height = height * 2;
        if (!target_texture->create(width, texture_height, DXGI_FORMAT_R8_UNORM, CTexture::CREATE_DYNAMIC))
          return;        
      }
...
CTexture* VideoTexture::getTexture() {
  assert(internal_data);
