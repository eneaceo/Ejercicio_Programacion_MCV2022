...
bool CGameCoreSkeleton::create(const std::string& res_name) {
  ...
  float custom_scale = json.value("scale", 1.0f);
  scale(custom_scale);
  ...
  if (json["ragdoll"].is_array()) {
	  ...
	  // todo: remove when exporting from 3d max
	  CalCoreSkeleton* calCoreSkeleton = getCoreSkeleton();
	  for (auto& core_bone : ragdoll_core.ragdoll_bone_cores) {
		  if (CalCoreBone * calCoreBone = calCoreSkeleton->getCoreBone(core_bone.bone))
		  {
			  if (CalCoreBone * parentCalCoreBone = calCoreSkeleton->getCoreBone(core_bone.parent_bone))
			  {
				  float height = (calCoreBone->getTranslationAbsolute() - parentCalCoreBone->getTranslationAbsolute()).length();
				  core_bone.height = height;
			  }
		  }
	  }
  }
  ...
  if (json.count("aabb_extra_factor"))
    aabb_extra_factor = loadVEC3(json, "aabb_extra_factor");
